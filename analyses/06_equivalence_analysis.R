...
## Duration ----
### L0
equivalence_dur_L0 <- data |> 
  ggplot(
    aes(
      x = practice_duration_logs_L0,
      y = practice_duration_survey_L0
    )
  ) +
  geom_point(shape = 1) +
  coord_cartesian(xlim = c(0, 5)) +
  scale_y_discrete(limits = levels(data$practice_duration_survey_L0))

### L2
equivalence_dur_L2 <- data |> 
  distinct(
    group_ID, .keep_all = T
  ) |> 
  ggplot(
    aes(
      x = practice_duration_logs_groupmean,
      y = practice_duration_survey_L2
    )
  ) +
  geom_point(shape = 1) +
  coord_cartesian(xlim = c(0, 5)) +
...
### L3
equivalence_dur_L3 <- data |> 
  distinct(
    school_ID, .keep_all = T
  ) |> 
  ggplot(
    aes(
      x = practice_duration_logs_schoolmean,
      y = practice_duration_survey_L3
    )
  ) +
  geom_point(shape = 1) +
  coord_cartesian(xlim = c(0, 5)) +
  scale_y_discrete(
    limits = levels(data$practice_duration_survey_L0)
    )

equivalence_dur <- equivalence_dur_L0 +
  plot_spacer() +
  equivalence_dur_L2 +
  equivalence_dur_L3 +
  plot_layout(ncol = 4)
...
blandtaltman_all <- wrap_elements(blandtaltman_length) + 
  wrap_elements(blandtaltman_freq) +
  wrap_elements(equivalence_dur) +
  plot_layout(nrow = 3)
