...
data_logs_student <- data_logs_session |> 
  ...
  mutate(
    practice_duration_logs = case_when(
      practice_duration_logs_months < 0.5 ~ 'Less than 1 month',
      (practice_duration_logs_months >= 0.5) & (practice_duration_logs_months < 2.5) ~ '1-2 months',
      (practice_duration_logs_months >= 2.5) & (practice_duration_logs_months < 4.5) ~ '3-4 months',
      practice_duration_logs_months >= 4.5 ~ 'Complete period'
    )
  ) |>
  dplyr::select(
    student_ID, 
